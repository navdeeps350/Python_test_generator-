...
class BranchTransformer(ast.NodeTransformer):
    ...
    def visit_FunctionDef(self, node):
        for n in ast.walk(node):
            if isinstance(n, ast.Assign):
                if isinstance(n.value, ast.Call) and n.value.func.id in function_names:
                    n.value.func.id = n.value.func.id + "_instrumented"
                elif isinstance(n.value, ast.BinOp):
                     if isinstance(n.value.right, ast.Call) and n.value.right.func.id in function_names:
    ...
    def visit_Return(self, node):
        if (isinstance(node.value, ast.Call)):
                if (isinstance(node.value.func, ast.Name)) and node.value.func.id in function_names:
                    node.value.func.id = node.value.func.id + "_instrumented"
        return node

    

b_transformer = BranchTransformer()
...
def evaluate_condition(num, op, lhs, rhs):  # type: ignore
    ...
    if op == "Eq" and (isinstance(lhs, int) or len(lhs) == 1):
        if lhs == rhs:
            distance_false = 1
        else:
            distance_true = abs(lhs - rhs)

    elif op == "Lt" and (isinstance(lhs, int) or len(lhs) == 1):
        if lhs < rhs:
    ...
    # ...
    elif op == 'Gt' and (isinstance(lhs, int) or len(lhs) == 1):
        if lhs > rhs:
            distance_false = lhs - rhs
        else:
            distance_true = rhs - lhs + 1 

    elif op == 'LtE' and (isinstance(lhs, int) or len(lhs) == 1):
        if lhs <= rhs:
            distance_false = rhs - lhs + 1
        else:
            distance_true = lhs - rhs

    elif op == 'GtE' and (isinstance(lhs, int) or len(lhs) == 1):
        if lhs >= rhs:
            distance_false = lhs - rhs + 1
        else:
            distance_true = rhs - lhs

    elif op == 'NotEq' and (isinstance(lhs, int) or len(lhs) == 1):
        if lhs == rhs:
            distance_true = 1
        else: 
